...
not otherwise required. The equality-augmented problem~\eqref{eq:eqp} adds only the column
selections $v \mapsto B_{\mathcal{F}}v$ and $w \mapsto B_{\mathcal{F}}^\top w$ used to form the
$p+1$ right-hand sides and the Schur complement of Section~\ref{sec:reduction}; these act on
the given $B$ and are independent of which backend implements $A$. Capturing exactly this contract as an operator protocol -- the same
