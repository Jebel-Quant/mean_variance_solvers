...
$O(\sqrt\kappa\,\log\varepsilon^{-1})$ iterations. The bound is the Chebyshev minimax
argument (the $k$-th CG error is majorised by the best degree-$k$ polynomial on
$[\lambda_{\min}, \lambda_{\max}]$ normalised at the origin, which is a scaled Chebyshev
polynomial~\cite[Lecture~38]{trefethenbau1997}), so clustered spectra beat it: CG converges
superlinearly once the extremal eigenvalues are resolved, which is exactly the behaviour the
synthetic study of Section~\ref{sec:results} measures. Because $A \succ 0$ forces every
...
\end{equation}
Rather than solve the indefinite system directly, eliminate $\lambda$ by the Schur
...
has full row rank; the multiplier is $\lambda = S^{-1}(c - B_{\mathcal{F}}v_0)$ and the
$p\times p$ solve is a dense Cholesky, negligible for the small $p$ typical of balance
conditions. The single-normalisation case $p=1$, $B=\mathbf{1}^\top$, $c=\beta$ recovers the
two solves $v_0, v_1 = A_{\mathcal{F}}^{-1}\mathbf{1}$ with the scalar
$\lambda = (\beta - \mathbf{1}^\top v_0)/(\mathbf{1}^\top v_1)$, and the pure-normalisation
case $b=0$, $\beta=1$ collapses to the single solve $x_{\mathcal{F}} = v_1/(\mathbf{1}^\top v_1)$.
