...
directly from their definitions; the external-solver benchmark below and the \texttt{shaw}
ill-posed study are two further scripts whose only additional dependency is SciPy (and, for
the benchmark, Clarabel). The algorithms are also released as the
...
under inexact solves, the warm-start single-step property, the \texttt{shaw} regularisation
sweep, and the adversarial family that
forces the fallback all run as continuous-integration checks. The geometric spectrum is the honest choice: its clustering
...
$\nncgReguErrLH{}$. The official problem tells the planted one's story: without the split,
termination is luck; with it, the guarantee holds.
