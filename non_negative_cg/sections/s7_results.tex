...
free-set size, $s$ the number of active-set outer steps (bounded by $(n+1)(1+p_{\max})$ on
generic data; Corollary~\ref{cor:outerbound}), $\kappa = \kappa(A_\alpha)$ the
...
\emph{(ii) Outer count is bounded by $O(n)$ and constant in practice.}
...
the codimension-one argument of Section~\ref{sec:nonneg}. Corollary~\ref{cor:outerbound}
provides an explicit polynomial ceiling: on generic data $s \leq (n+1)(1+p_{\max}) = 4(n+1)$
for the default $p_{\max}=3$, a linear bound in $n$ that supersedes the $2^n$ finiteness
ceiling of Theorem~\ref{thm:termination}. In practice $s$ is far smaller: sweeping
$n \in [\nncgOuterNMin, \nncgOuterNMax]$ at fixed $\kappa = 10^4$ gives a mean outer count
never exceeding $\nncgOuterMaxMean$, with a $\log$--$\log$ slope of \nncgOuterSlope{} (essentially
constant), confirming that $s$ is $O(1)$ on this family rather than $O(n)$.
...
\textcolor{blue}{
\emph{Preconditioner reuse: \texttt{Nystrom} versus \texttt{GlobalNystrom}.} The sketch-once
design of Section~\ref{ssec:precond} is tested on the planted spectral-gap family of
github.com/Jebel-Quant/nncg's solver-comparison notebook: $\nncgPrecondN{}$ dominant-over-tail
eigenvalues, sketch rank $\nncgPrecondRank{}$, solved against $\nncgPrecondNSolves{}$
independent right-hand sides sharing one operator and one solver instance per method (so
\texttt{GlobalNystrom}'s cache persists across solves exactly as Proposition~\ref{prop:warm}'s
warm-started re-solves would reuse it). Figure~\ref{fig:nncg_precond} plots cumulative wall
time against the number of solves for \texttt{CG}, \texttt{Jacobi}, \texttt{Nystrom}, and
\texttt{GlobalNystrom}: all four reach the identical solution on every right-hand side (max
disagreement $\nncgPrecondMaxError{}$), so only the cost differs. \texttt{GlobalNystrom}'s
cumulative cost falls below \texttt{Nystrom}'s already by solve $\nncgPrecondCrossover{}$ and
finishes $\nncgPrecondFinalSpeedup{}\times$ ahead ($\nncgPrecondGlobalFinalMs{}$\,ms versus
$\nncgPrecondNystromFinalMs{}$\,ms), confirming that paying the larger whole-operator sketch
once is cheaper than resketching every free block, on every solve, once the same operator is
reused often enough.
}
...
\caption{\textcolor{blue}{Cumulative wall time over $\nncgPrecondNSolves{}$ repeated solves of
the same $n=\nncgPrecondN{}$ spectral-gap operator (sketch rank $\nncgPrecondRank{}$), one line
per inner solver. \texttt{GlobalNystrom} overtakes \texttt{Nystrom} at solve
$\nncgPrecondCrossover{}$ and stays ahead, since its one-time full-operator sketch is amortised
across every later free-block solve rather than repaid on each one.}}
...
\textcolor{blue}{
\emph{A real, non-synthetic check.} The external-baseline comparison above is on a planted
family designed to be fair to Lawson--Hanson and Clarabel alike; a genuine hyperspectral
unmixing problem confirms the same ordering on real data. The Urban scene (HYDICE sensor,
$\nncgHsUrbPixels{}$ pixels, $\nncgHsUrbBands{}$ clean bands, $\nncgHsUrbMaterials{}$ named
materials: Asphalt, Grass, Tree, Roof, Metal, Dirt) poses the fully-constrained least-squares
abundance problem $\min_x \tfrac12 x^\top A x - b_i^\top x$ s.t.\ $x \geq 0$, $\mathbf{1}^\top x
= 1$, $A = M^\top M$, independently at every pixel, with $M$ the per-material mean reference
spectrum. At $n = \nncgHsUrbMaterials{}$ material classes the loop and Clarabel are essentially
matched ($\nncgHsUrbSpeedupWall{}\times$) --- consistent with Table~\ref{tab:complexity}'s
per-step costs crossing over only once $n$ grows enough for Clarabel's $O(n^3)$ factorisation
to outweigh the active-set loop's $O(km)$ mat-vecs, which a handful of material classes is far
too small to exercise; FISTA is not competitive at any $n$
($\nncgHsUrbSpeedupFista{}\times$ slower, needing a mean of $\nncgHsUrbFistaIters{}$ and up to
$\nncgHsUrbFistaItersMax{}$ iterations to the same fixed-point-residual tolerance). Raster-order
warm starting cuts total outer steps by $\nncgHsUrbSpeedupOuter{}\times$
($\nncgHsUrbFracSingle{}\%$ of pixels certify in a single warm-started outer step), and cold and
warm starts agree to $\nncgHsUrbAgree{}$. Figure~\ref{fig:nncg_hyperspectral_urban} shows the
recovered dominant-material map: the recovered abundances trace out the scene's roads,
rooftops, and running track, rather than a speckled, uninterpretable field.
}

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\textwidth]{graphs/nncg_hyperspectral_urban}
\caption{\textcolor{blue}{Urban hyperspectral scene ($\nncgHsUrbPixels{}$ pixels,
$\nncgHsUrbBands{}$ bands, $\nncgHsUrbMaterials{}$ materials): dominant recovered material per
pixel (left) and the outer-iteration count of the active-set loop, cold versus warm-started
(right).}}
\label{fig:nncg_hyperspectral_urban}
\end{figure}
