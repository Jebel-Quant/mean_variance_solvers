...
necessary and sufficient. Introduce a multiplier $s \geq 0$ for $x \geq 0$. Stationarity of
...
balance conditions. The KKT system is now a \emph{mixed} complementarity problem: the free
...
used later. First, the linear system $A_{\mathcal{F}}\,x_{\mathcal{F}} = b_{\mathcal{F}}$
solved on any candidate free set is well posed, so CG applies at every outer step
(Section~\ref{sec:reduction}). Second, $\mathrm{LCP}(A,\,{-b})$ with a $P$-matrix has a unique
