...
sets $\mathcal{F}$. This means that once $\mathcal{F}$ is fixed, the sign constraints drop out and what remains on $\mathcal{F}$ is exactly the linear system~\eqref{eq:innerspd} guaranteed well posed above.
Section~\ref{sec:reduction} makes this reduction precise, together with the closed-form elimination of $\lambda$ for the equality-augmented problem~\eqref{eq:eqp}.
