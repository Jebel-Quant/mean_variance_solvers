...
zero case is not excluded, cf.\ Remark~\ref{rem:inexact}(i)); either way $\tilde x_i \ge
...
Three comments. (i) Exactly-zero components are not excluded: the threshold $\varepsilon$
absorbs them on both sides of every test, so the lemma holds even under degeneracy, and the
$\varepsilon$-thresholded algorithm as implemented matches the exact sign tests of
Theorem~\ref{thm:termination} as analysed. (ii) When CG is stopped at a relative residual,
$\rho \le \varepsilon_{\mathrm{cg}} \lVert b \rVert_2$, condition~\eqref{eq:inexactcond}
becomes a bound on $\varepsilon_{\mathrm{cg}}$ alone. (iii) The margin $\mu$ is not computable
a priori, so the lemma is a license, not a recipe: it guarantees that \emph{some} finite
tolerance reproduces the exact trajectory, and in practice a residual tolerance a couple of
orders of magnitude below $\varepsilon$ suffices (Section~\ref{sssec:inexact} verifies
trajectory-for-trajectory agreement at $\varepsilon_{\mathrm{cg}} = 10^{-10}$,
$\varepsilon = 10^{-8}$).
\end{remark}
