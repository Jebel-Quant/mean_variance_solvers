...
$f_{\mathcal{F}}$; CG returns an iterate accurate only to its residual tolerance. The
...
coincides with the decision the exact solve would make, so the inexact loop inherits the
theorem wholesale.
...
and $\lVert A_{i,\mathcal{F}} \rVert_2 \le \lVert A \rVert_2 = \lambda_{\max}(A)$, so
...
(iii) The margin $\mu$ is not computable a priori; the lemma is a license, not a recipe: it
...
plain), but the empirical picture is simple: the warm-started outer count tracks the support
drift $|\mathcal{F} \,\triangle\, \mathcal{F}'|$ and collapses to one whenever the drift is
zero, exactly as the proposition prescribes (Section~\ref{ssec:synthetic}). A direct inner
solver profits only from the warm free set, having no analogue of an initial guess; the
matrix-free CG inner solver profits from both, which is where warm-started parametric sweeps obtain their largest speed-ups.
