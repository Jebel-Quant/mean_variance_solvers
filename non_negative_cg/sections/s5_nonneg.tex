...
algorithm of Júdice and Pires~\cite{judice1994}: a batch exchange safeguarded by Murty's
least-index single pivot~\cite{murty1988}, already finite on the strictly monotone
($A \succ 0$) LCP with no non-degeneracy assumption. We claim no novelty for that
construction; Theorem~\ref{thm:termination} restates it in active-set language for
self-containedness. The contribution is what Table~\ref{tab:novelty} isolates, and its crux is
the first row: classical block-pivoting termination assumes each principal-block system is
solved \emph{exactly}, whereas the matrix-free regime solves it only to a tolerance.
Lemma~\ref{lem:inexact} closes exactly this gap---once the inner residual is tied to the
decision margin, the inexact loop makes the identical primal and dual sign decisions as the
exact loop, so the Júdice--Pires count transfers verbatim. Without it, block pivoting and
Krylov inner solves do not obviously compose: a tolerance-level solve can flip a sign test and
reintroduce the cycling the fallback exists to prevent.
...
\midrule
Inner solve  & exact principal-block solve & inexact matrix-free CG, guarantee preserved (Lemma~\ref{lem:inexact}) \\
...
(Section~\ref{sssec:inexact} verifies trajectory-for-trajectory agreement at
