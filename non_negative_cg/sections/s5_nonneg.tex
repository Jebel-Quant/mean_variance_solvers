...
\emph{dual step} inspects the reduced gradient $s = Ax - b$ (minus $\lambda\mathbf{1}$ in the
...
\Loop
  ...
  \State $s_i \leftarrow (Ax)_i - b_i$
    \hfill\textit{(reduced gradient; subtract $\lambda$ in the equality-augmented case)}
...
problem~\eqref{eq:eqp} with the reduced gradient of line~5 adjusted by $\lambda\mathbf{1}$.
\end{theorem}
...
has a unique solution to which CG converges (Proposition~\ref{prop:cg}).
...
\[
  ...
  \qquad \nabla f(x) = A x - b,
\]
where $C$ is the feasible set -- the non-negative orthant for~\eqref{eq:bqp}, so
$\Pi_C(y) = \max(y, 0)$ componentwise, or the affine simplex slice $\Delta_\beta$ for the
equality-augmented problem~\eqref{eq:eqp}, projected onto in $O(n\log n)$ by a single
sort-and-threshold pass~\cite{duchi2008,condat2016}. The step size is $\tau = 1/L$ with
