...
The split works twofold. It lowers $\kappa$ and hence the iteration count of both solvers
...
The bound is immediate: $P_{\mathcal{F}}^{-1/2} A_{\mathcal{F}} P_{\mathcal{F}}^{-1/2}$ is SPD
and similar to $P_{\mathcal{F}}^{-1} A_{\mathcal{F}}$, so the two share the spectrum entering
$\kappa_P$, and Proposition~\ref{prop:cg} applies to the scaled operator verbatim.
...
negligible cost. The standard choices sort by exactly this property. The Jacobi preconditioner
$P = \operatorname{diag}(A)$ restricts exactly, $(\operatorname{diag} A)_{\mathcal{F}} =
\operatorname{diag}(A_{\mathcal{F}})$, so $P_{\mathcal{F}}$ is the free-set slice of a single
vector computed once; for $A = M^\top M$ that vector is the squared column norms of $M$,
available without forming $A$, and its apply is $O(|\mathcal{F}|)$. It carries no per-step
setup and is valid on every free set the loop visits, the natural default when $A$ has a
widely varying diagonal. A factorised preconditioner (incomplete Cholesky, $P = LL^\top$) does
\emph{not} restrict: in general $(P_{\mathcal{F}})^{-1} \neq (P^{-1})_{\mathcal{F}}$, and the
factor of the full $P$ is not a factor of $P_{\mathcal{F}}$, so honouring $A_{\mathcal{F}}$
would require refactorising on each free set, reintroducing the assembly the method avoids;
such preconditioners pay off here only once the free set stabilises, as it does near
convergence and across the support-stable warm starts of Proposition~\ref{prop:warm}, where
one factor serves many outer steps. Between these sits the low-rank-plus-diagonal
preconditioner $P = \operatorname{diag}(d) + U\Delta U^\top$ built from a few ($r$) dominant
eigenpairs of $A$~\cite{saad2003iterative}: it restricts consistently in its \emph{apply},
...
Woodbury identity, precisely the direct free-block solve that the companion random-matrix
paper adopts as its backend~\cite{schmelzer2026rmt}. It captures the leading spectral structure
a diagonal misses, and when $A$ is itself diagonal-plus-low-rank the preconditioner \emph{is}
$A$, giving $\kappa_P = 1$ and a direct inner solve that bypasses CG altogether.

\textcolor{blue}{
  The \texttt{nncg} method builds this low-rank-plus-diagonal preconditioner by randomized Nystr\"om
sketching~\cite{halko2011}, $A \approx U\Lambda U^\top + \mu(I - UU^\top)$ for an
orthonormal $U \in \mathbb{R}^{n \times r}$, captured eigenvalues $\Lambda$ and a scalar
tail shift $\mu$, the constant-diagonal case $d = \mu\mathbf{1}$, $\Delta = \Lambda - \mu I$
of the preconditioner above, in two variants that trade off exactly where the free set changes. The first method, \texttt{Nystrom} sketches the \emph{current} free block $A_{\mathcal{F}}$ on every outer step consisting of a handful of matrix-free products against $A_{\mathcal{F}}$ plus a QR and a
small eigendecomposition. This means it is always built on the operator the loop is about to solve, at the price of repeating that work whenever $\mathcal{F}$ changes. In contrast, the \texttt{GlobalNystrom} approach
sketches the \emph{full} operator $A$ once, and exploits that restricting a rank-$r$
factorisation to a principal submatrix is exact,
...
U[\mathcal{F},:]$. This means that every later free set reuses the one global sketch by masking rows rather than resketching. The masked $U_{\mathcal{F}}$ is no longer orthonormal, so its inverse needs the general Sherman--Morrison--Woodbury identity rather than \texttt{Nystrom}'s specialised identity-plus-projection form, built once per free set in $O(|\mathcal{F}|\,r + r^3)$ and applied in $O(|\mathcal{F}|\,r)$ thereafter. The trade mirrors the factorised-preconditioner case above: \texttt{GlobalNystrom} pays its (larger, whole-operator) sketch cost once and
amortises it across every outer step of a solve and across the support-stable warm-started re-solves of Proposition~\ref{prop:warm} as well, exactly where a resketched-per-block \texttt{Nystrom} pays repeatedly for work \texttt{GlobalNystrom} does once.
}
...
preconditioning changes only how fast $\rho$ falls, not $A$, not $\rho$, and not the decision
margin $\mu$.

Two remarks connect the subsection to the rest of the paper. The regularising
...
And preconditioning is a lever the Krylov inner solver has that the projected-gradient
...
active-set/CG loop over projected gradient rather than closing it. The companion
