...
The cost is $O(n^2)$, all of it in $x_u = J(J\T a)$ --- two matrix--vector products
with a dense $n \times n$ matrix, $J$ having lost its triangularity at the first
update --- with $O(nk + k^2)$ for everything after. Re-deriving the factors for the
...
an overhead floor --- a hit there costs what its dozen array operations cost to
