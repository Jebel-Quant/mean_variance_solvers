...
\end{align}
Everything the iteration computes is a consequence of these two lines, so it is
worth reading them slowly. Equation~\eqref{eq:inv1} is equivalent to
...
\begin{proof}
(i) From~\eqref{eq:inv2}, $A\T J = [\,R\T\ \ 0\,]$, so
$A\T G^{-1} A = A\T J J\T A = [\,R\T\ 0\,][\,R\T\ 0\,]\T = R\T R$. Uniqueness of
the Cholesky factor of a positive definite matrix up to the signs of the diagonal
entries gives the last clause, since two upper triangular matrices with the same
Gram matrix differ by a left factor $\mathrm{diag}(\pm1)$.

(ii) $J_2\T A = 0$ places $\range(J_2) \subseteq \nullsp(A\T)$; the inclusion is
an equality by dimension, $J$ being nonsingular and $A$ of full column rank. The
basis is $G$-orthonormal because $J_2\T G J_2 = I_{n-k}$ is the trailing block
of~\eqref{eq:inv1}.
...
factored form, at no cost beyond the $n \times n$ array, and it is what turns the
search directions of Section~\ref{sec:iteration} into single matrix--vector
products.
...
the QR factorisation of $R_G^{-\top} A$. That holds the same information --- $J$
is $J_0 Q$ --- and stores no more numbers. But every use of $J$ in
...
this does not matter, and it is the licence for the implementation to choose its
reduction on grounds of speed alone.
...
iteration's decisions --- which constraint is most violated, whether the step is
full or partial, which constraint is dropped --- are functions of $z$ and $r$
...
trajectories against each other. Section~\ref{ssec:identities} settles the weaker
statement that is checkable against one: that the $z$ and $r$ the solver computes
agree with the representation-free closed forms~\eqref{eq:zclosed}
