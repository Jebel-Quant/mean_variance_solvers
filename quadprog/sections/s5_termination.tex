...
Hypothesis (a) is $t_2 = \infty$ and (b) is $t_1 = \infty$, so the condition the
implementation tests --- neither step limit finite --- is literally the hypothesis
of Proposition~\ref{prop:farkas}. One caveat: it also needs the entering
constraint to be genuinely violated, which exact arithmetic gives free and finite
precision does not (Section~\ref{sec:arithmetic}).
