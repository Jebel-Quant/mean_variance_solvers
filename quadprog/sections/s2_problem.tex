...
If $\Omega \neq \emptyset$ then~\eqref{eq:qp2} has exactly one solution.
\end{proposition}

\begin{proof}
Since $G \succ 0$, write $G = R_G\T R_G$ and complete the square:
$f(x) = \tfrac12\norm{R_G(x - G^{-1}a)}^2 - \tfrac12 a\T G^{-1} a$. So $f$ is
coercive --- $f(x) \to \infty$ as $\norm{x} \to \infty$ --- and continuous, and
$\Omega$ is closed and nonempty, so a minimiser exists. It is unique because $f$
is strictly convex: for $x \neq y$ and $\theta \in (0,1)$,
$f(\theta x + (1-\theta)y) = \theta f(x) + (1-\theta)f(y)
- \tfrac12\theta(1-\theta)(x-y)\T G (x-y) < \theta f(x) + (1-\theta) f(y)$,
and $\Omega$ is convex, so two distinct minimisers would produce a feasible point
of strictly smaller value.
\end{proof}
...
minimiser by the argument of Proposition~\ref{prop:unique} applied to the affine
...
The Lagrangian dual of~\eqref{eq:qp2} is the concave quadratic program
\begin{equation}
  \max_{\lambda_i \ge 0 \ (i > m_{\mathrm{eq}})} \;
  \psi(\lambda) := -\tfrac12 (a + C\lambda)\T G^{-1} (a + C\lambda)
   + b\T\lambda .
  \label{eq:dualfn}
\end{equation}
...
\begin{proof}
For fixed $\lambda$, $L(\cdot,\lambda)$ is a strictly convex quadratic minimised
where $Gx = a + C\lambda$, i.e.\ at $x(\lambda) = G^{-1}(a + C\lambda)$. Writing
$s = a + C\lambda$,
\[
  \psi(\lambda) = L(x(\lambda),\lambda)
  = \tfrac12 s\T G^{-1} s - a\T G^{-1} s - \lambda\T C\T G^{-1} s + b\T\lambda
  = \tfrac12 s\T G^{-1}s - s\T G^{-1} s + b\T\lambda,
\]
using $a + C\lambda = s$, which is~\eqref{eq:dualfn}. For the second claim,
\eqref{eq:stat} says $x = x(\lambda)$, so $s = Gx$ and
$\psi(\lambda) = -\tfrac12 x\T G x + b\T\lambda$; and \eqref{eq:comp} together
with $c_i\T x = b_i$ on the equalities gives
$b\T\lambda = \lambda\T C\T x = (Gx - a)\T x = x\T G x - a\T x$. Adding,
$\psi(\lambda) = \tfrac12 x\T Gx - a\T x = f(x)$.
\end{proof}
...
minimiser and the dual objective at the current multipliers. The method is a dual
ascent: Proposition~\ref{prop:increment} will show the value strictly increases,
and Proposition~\ref{prop:dual} is what makes that a statement about progress
towards optimality rather than an accident of bookkeeping. It also gives the
sandwich $\psi(\lambda) \le f(x^\star) \le f(\tilde x)$ for any dual-feasible
$\lambda$ and primal-feasible $\tilde x$, so the method approaches the optimal
value from below --- the mirror image of a primal active-set method, which
