...
entering constraint and its accumulated $u_*$, and the loop repeats. If both
...
The increment~\eqref{eq:increment} is exact, so the objective can be carried as a
running total at the cost of one multiplication per step instead of an $O(n^2)$
re-evaluation of the quadratic. It is expressed in quantities the iteration
already holds, $n_*\T z$ being needed for $t_2$ anyway. And it handles the
reversed step of Remark~\ref{rem:reverse} without a
