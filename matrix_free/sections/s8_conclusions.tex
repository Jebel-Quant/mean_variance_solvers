...
covariance matrix is unnecessary: the problem reduces to one symmetric positive definite
linear solve, with non-negativity enforced either by an active-set loop or by projecting
gradient steps directly onto the probability simplex. Both strategies work entirely with
...
The simplex-projection comparator is $19\times$ slower than CG at $n=3000$,
as its $O(\kappa)$ iteration count grows with the condition number while CG's $O(\sqrt\kappa)$
count does not. Both strategies extend naturally to the
mean-variance problem with a return tilt, which requires solving two SPD systems and
recovering the budget multiplier analytically, as outlined in Section~\ref{sec:pd}.
