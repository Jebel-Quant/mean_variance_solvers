...
solver on the modified active set at each step. The treatment covers a general system
of linear balance conditions $Bw = c$ (group budgets, factor neutralities, a target
beta) with the budget constraint as the leading case: the $p$ multipliers are
eliminated analytically through a $p \times p$ Schur complement at the cost of $p+1$
matrix-free solves (Section~\ref{sec:pd}). This constraint-handling layer (the
loop itself, its unconditional finite-termination guarantee, and its robustness to
inexact inner solves) is developed and analysed in full generality in the companion
paper~\cite{schmelzer2026nncg}; here it is instantiated on the portfolio problem.
Because CG is a Krylov method, it builds
...
stepping outside the QP abstraction. Second, recovering the balance multipliers
analytically (reducing the long-only problem to a handful of SPD linear solves rather
than a constrained QP) exploits the low-rank ($p \ll n$) structure of the equality
...
problem: the matrix-free reduction exploits its specific structure (a few SPD
solves with a low-rank system of balance equalities) and \emph{inequality} constraint
sets such as sector
