...
budget instantiation ($p = 1$) implemented in the companion package; the general
$p > 1$ balance system replaces the single right-hand side with the $p$ columns of
$B_a^\top$ and the scalar renormalisation with the $p\times p$ Schur solve of
Section~\ref{sec:pd}, as benchmarked in the balance-systems panel of
Table~\ref{tab:sp500}.
...
FISTA variant~\cite{beck2009}, available in the companion
package~\cite{fast_minimum_variance}, empirically needs $1.5$--$1.8\times$ fewer
