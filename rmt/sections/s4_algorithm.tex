...
The outer loop, its finite-termination proof, and the batch/least-index pivot
schedule that makes it robust are developed in full generality in the companion
paper~\cite{schmelzer2026nncg}, of which the RMT problem is the instance
$A = C_0$, $b = \rho\hat\mu$. Theorem~4.1 there gives finite termination at the
unique global minimiser with no non-degeneracy assumption, and licenses any
\emph{direct} factorisation of the free-set system as the inner solve: the
Woodbury solve developed below is exactly such a solver, so termination is
inherited.
...
active set and weights, cutting the number of outer steps. Along the frontier the
