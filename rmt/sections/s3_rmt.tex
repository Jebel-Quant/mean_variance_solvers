...
over $T$ periods and $\hat\Sigma = X^\top X / T$ the sample covariance. This
section replaces the noise eigenvalues of $\hat\Sigma$ with a single value,
producing the low-rank-plus-identity estimator on which the portfolio problem of
Section~\ref{sec:problem} is posed.
...
clipping, estimator introduced by Laloux et al.~\cite{laloux1999} and studied
in depth by Bun, Bouchaud and Potters~\cite{bun2017}. It was later popularised
among quantitative-finance practitioners by L\'opez de
Prado~\cite{lopezdeprado2018}, whose accessible implementations brought the
method to a wide audience; the estimator itself is due to the earlier
random-matrix literature.
