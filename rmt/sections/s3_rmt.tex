...
therefore applied to $C$, whose Marchenko--Pastur bulk is a clean band. This
section supplies the $C_0$ that the standardised problem of
Section~\ref{sec:problem} is posed against, and establishes the
scalar-identity-plus-low-rank structure that makes the solve of
Section~\ref{sec:algorithm} possible.
...
This is the $C_0$ the standardised problem of Section~\ref{sec:problem} requires,
and it is $C_0$ that the solve runs against: the scalar-identity-plus-low-rank
form above, not the diagonal-plus-low-rank $\Sigma_0 = D^{1/2}C_0D^{1/2}$
obtained by rebuilding the covariance.
...
literature cited above; Section~\ref{ssec:oos} reports out-of-sample
