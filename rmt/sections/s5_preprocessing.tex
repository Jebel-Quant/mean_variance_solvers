...
$X_s = X D^{-1/2}$. Two paths are available. Throughout this section $q$ denotes
the oversampling parameter of the randomised sketch; the symbol $p$ is reserved
for the number of balance constraints as elsewhere in the paper.
...
\begin{algorithmic}[1]
\Require Standardised returns $X_s = X D^{-1/2} \in \mathbb{R}^{T \times n}$; target rank $k$; oversampling $q \geq 5$
\Ensure  Signal eigenvectors $U_k \in \mathbb{R}^{n \times k}$,
         eigenvalues $\Lambda_k \in \mathbb{R}^k$, bulk mean $\bar\mu$
\State $\Omega \leftarrow \mathcal{N}(0, I_{n \times (k+q)})$ \Comment{random test matrix}
\State $Y \leftarrow X_s^\top(X_s \Omega)$ \Comment{$n \times (k+q)$; two matrix-vector products with $X_s$, $O(nT(k+q))$}
\State $Q \leftarrow \mathrm{orth}(Y)$ \Comment{orthonormal basis, $O(n(k+q)^2)$}
\State $G \leftarrow (X_s Q)^\top(X_s Q) / T$ \Comment{$(k+q) \times (k+q)$ projected correlation; no $X_s^\top X_s$ formed}
\State $[\hat U, \hat\Lambda, \sim] \leftarrow \mathrm{eigh}(G)$; sort descending
...
Algorithm~\ref{alg:rsvd} costs $O(nT(k+q))$ FLOPs and $O(n(k+q))$ storage; no
...
$\hat\Sigma$ with Gaussian test matrix $\Omega$ and oversampling $q \geq 2$,
Theorem~10.6 of~\cite{halko2011} gives, in expectation,
\[
  \mathbb{E}\,\bigl\lVert (I - P)\hat\Sigma \bigr\rVert_2
  \;\leq\;
  \Bigl(1 + \sqrt{\tfrac{k}{q-1}}\Bigr)\,\lambda_{k+1}
  \;+\; \frac{e\sqrt{k+q}}{q}\Bigl(\textstyle\sum_{j>k}\lambda_j^2\Bigr)^{1/2}
  \;=:\; \epsilon_{k,q}\,,
\]
...
The default $q = 10$ used in the experiments comfortably exceeds the $q \geq 2$
the bound requires. The bound $\epsilon_{k,q}$ is governed by the correlation
eigenvalues at and below
...
$G = Q^\top X^\top XQ$ (line~4, computed as $(XQ)^\top(XQ)/T$). Reading $X$
...
\paragraph{Benchmark}
On S\&P~500 ($n=494$, $T=1213$, $k=17$, $q=10$) the randomised SVD is $1.9\times$
faster than the dense path ($0.006$\,s vs $0.011$\,s), reduces storage from
$O(n^2)$ to $O(n(k+q))$ (a factor of $18$, or $29$ counting only the returned
$O(nk)$ factors that the solver retains), and aligns the top-$k$ signal subspace
to within a Frobenius error
...
$O(n(k+q))$ requirement (about $3$\,MB at $n=3000$, $k=125$) stays within
typical L3.
