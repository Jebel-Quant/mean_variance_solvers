...
eigendecomposition ($O(n^3)$). Total cost $O(n^2 T + n^3)$; $0.011$\,s at
$n = 494$, $T = 1213$ on the benchmark hardware (Table~\ref{tab:preprocessing}).
...
\bottomrule
\end{tabular}
\caption{Preprocessing paths for S\&P~500 ($n=494$, $T=1213$, $k=17$, $p=10$).
Subspace error: $\norm{UU^\top - \hat{U}\hat{U}^\top}_F$, measuring alignment of the
top-$k$ signal subspaces.  The randomised SVD is $1.9\times$ faster and requires
$29\times$ less storage.}
\label{tab:preprocessing}
\end{table}

The randomised SVD is $1.9\times$ faster than the dense path at $n=494$, and the
advantage grows with $n$: at $n=3000$ (Table~\ref{tab:scaling}) the dense
eigendecomposition takes $1.12$\,s against $0.056$\,s for the randomised path, a
$20\times$ saving. The subspace error
$\norm{UU^\top - \hat{U}\hat{U}^\top}_F \approx 2.5 \times 10^{-2}$ is about
$0.6\%$ alignment per eigenvector column (relative to the subspace scale
$\sqrt{k} \approx 4.1$), an order of magnitude below the $k \pm 1$ threshold
