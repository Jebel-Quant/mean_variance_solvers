...
\emph{Second, randomised-SVD preprocessing.} The companion paper uses a dense
eigendecomposition of $\hat\Sigma$ for simplicity; at large $n$ this costs
$O(n^2 T + n^3)$ and re-introduces the $n^2$ storage the matrix-free approach
aims to avoid. A randomised truncated SVD applied directly to $X$ computes the
...
(Section~\ref{sec:results}), with optimality cross-checks of the returned
weights and an exact critical-line sweep built on the same Woodbury kernel that
traces every frontier breakpoint (Section~\ref{ssec:cla}). A $k$-sensitivity
...
Section~\ref{sec:problem} states the long-only problem and the active-set
reduction. Section~\ref{sec:algorithm} develops the Woodbury portfolio
