...
restriction (Section~\ref{sec:algorithm}). It slots into the primal-dual
active-set loop of~\cite{schmelzer2026nncg}, whose Schur elimination of the $p$
linear balance constraints $Bw=c$ (the budget $\mathbf{1}^\top w=1$ as the
leading case) and whose finite-termination guarantee it inherits unchanged. We
derive the exact crossover
...
satisfied throughout the long-only equity regime ($k/n_a$ between roughly $0.33$
and $1$ in our experiments), and we account for the total pipeline cost over
...
The division of labour with~\cite{schmelzer2026nncg} is deliberate: that paper
develops the outer loop, its finite-termination guarantee, and the Schur
elimination of the balance constraints in domain-neutral generality, and admits
any direct free-set factorisation as the inner solver. We use it as printed and
supply that factorisation (Algorithm~\ref{alg:woodbury}) together with the rSVD
preprocessing that feeds it (Section~\ref{sec:preprocessing}).

\paragraph{Outline}
Section~\ref{sec:problem} states the long-only problem and reduces it to the
standardised coordinates. Section~\ref{sec:rmt} constructs the cleaned estimator
that supplies its system matrix.
