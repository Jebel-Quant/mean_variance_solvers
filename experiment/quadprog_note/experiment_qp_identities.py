...
def emit(w, n_problems, n_states) -> None:
    ...
    # The per-identity rows were a table in the paper and are no longer: a
    # 28-row verification of the authors' own algebra against their own code is a
    # test suite, and the paper now states the worst residual in one line instead.
    # They are still written, because reading them is how one finds out *which*
    # identity moved when the worst residual changes.
    path = TABLES / "quadprog_identities.tex"
    path.write_text(
        "% Generated by quadprog_note/experiment_qp_identities.py -- do not edit by hand.\n"
        "% Not input by the paper; kept because it says which identity is worst.\n"
